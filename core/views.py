    ...
    def get_queryset(self):
        ...
        if sort_by == "discussed":
            queryset = Post.objects.annotate(
                reply_count=Count("post_replies")
    ...
    if request.method == "POST":
        ...
        if reply_id:  # This handles the edit reply form
            try:
                ...
                # Ensure that only the author can edit the reply
                if reply.author == request.user:
                    reply_form = ReplyForm(data=request.POST, instance=reply)
                    if reply_form.is_valid():
                        reply_form.save()
                        messages.success(
                            request, "Reply updated successfully."
                            )
                    else:
                        messages.error(request, "Failed to update reply.")
                else:
                    messages.error(
                        request, "You are not allowed to edit this reply."
                        )
...
def reply_delete(request, slug, reply_id):
    ...
    if reply.author == request.user:
        ...
        messages.add_message(
            request, messages.ERROR, "You can only delete your own reply!"
        )
